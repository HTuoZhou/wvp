...
CREATE TABLE `wvp_media_server` (
                                ...
                                `status` tinyint(1) NOT NULL DEFAULT true NULL COMMENT '是否在线',
                                `default_server` tinyint(1) NOT NULL COMMENT '是否默认服务器',
                                `create_time` datetime NOT NULL COMMENT '创建时间',
                                `update_time` datetime NOT NULL COMMENT '更新时间',
                                `deleted` int NOT NULL DEFAULT '0' COMMENT '删除标识（0、逻辑删除 1、物理删除）',
                                `version` int NOT NULL DEFAULT '0' COMMENT '版本号',
...
CREATE TABLE `wvp_device_channel` (
                                  ...
                                  `parent_id` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT '父级id',
                                  `safety_way` int DEFAULT NULL COMMENT '信令安全模式（0、不采用2、S/MIME签名方式 3、S/MIME加密签名同时采用方式 4、数字摘要方式）',
                                  `register_way` int DEFAULT NULL COMMENT '注册方式（1、符合IETFRFC3261标准的认证注册模式 2、基于口令的双向认证注册模式 3、基于数字证书的双向认证注册模式）',
                                  `cert_num` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT '证书序列号',
                                  `certifiable` int DEFAULT NULL COMMENT '证书有效标识（0、无效 1、有效）',
                                  ...
                                  `secrecy` int DEFAULT NULL COMMENT '保密属性（0、不涉密 1、涉密）',
                                  ...
                                  `ptz_type` int DEFAULT NULL COMMENT '云台类型',
                                  ...
                                  `sub_count` int DEFAULT NULL COMMENT '子设备数',
